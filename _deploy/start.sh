...
if ! ss -tlnp 2>/dev/null | grep -q "127.0.0.1:31334"; then
  ...
  for i in $(seq 1 20); do
    if curl -s -m 1 http://127.0.0.1:31334/healthz >/dev/null 2>&1; then
  ...
  echo "[webui] backend already running on 127.0.0.1:31334"
fi
